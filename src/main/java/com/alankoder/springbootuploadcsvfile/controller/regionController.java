...
import com.alankoder.springbootuploadcsvfile.service.regionCSVService;
...
public class regionController {
    ...
    @Autowired
    regionCSVService fileService;

    @PostMapping("/uploadRegionStateFile")
    public ResponseEntity<ResponseMessage> uploadFile(@RequestParam("file") MultipartFile file) {
        String message = "";

        if (regionCSVHelper.hasCSVFormat(file)) {
            try {
    ...
    @GetMapping("/regionStates")
    public ResponseEntity<List<regionState>> getAllRegions() {
        try {
            List<regionState> listRegions = fileService.getAllRegions();
